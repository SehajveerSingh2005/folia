...
// Default aesthetic image
const DEFAULT_IMAGE_URL = "https://images.unsplash.com/photo-1542273917363-3b1817f69a2d?q=80&w=2674&auto=format&fit=crop";
...
const ImageWidget = ({ data, onUpdate, isEditable }: ImageWidgetProps) => {
    ...
    // Initial load: Set default image if empty
    useEffect(() => {
        if (!data.url) {
            onUpdate({ ...data, url: DEFAULT_IMAGE_URL });
        }
    }, []);
    ...
    const handleSave = () => {
        ...
        // Basic fix for Unsplash photo page URLs
        if (finalUrl.includes('unsplash.com/photos/')) {
            setError("Use 'Copy Image Address' on the image, not the page URL.");
            return;
        }
            ...
            <div className="h-full w-full flex flex-col items-center justify-center p-4 bg-muted/20 rounded-lg">
                <div className="w-full max-w-[240px] space-y-4">
                    <div className="text-center space-y-1">
                        <p className="text-xs font-medium text-muted-foreground">Image Source</p>
                    </div>

                    <div className="flex gap-2 justify-center">
                        <Button variant="outline" size="sm" className="h-7 text-[10px] flex-1" asChild>
                            <a href="https://in.pinterest.com/search/pins/?q=aesthetic%20wallpaper" target="_blank" rel="noreferrer">Pinterest</a>
                        </Button>
                        <Button variant="outline" size="sm" className="h-7 text-[10px] flex-1" asChild>
                            <a href="https://unsplash.com/s/photos/editorial" target="_blank" rel="noreferrer">Unsplash</a>
                        </Button>
                    </div>
                    ...
                    <div className="space-y-2 pt-2 border-t border-border/50">
                        ...
                        {error && <p className="text-[10px] text-destructive leading-tight text-center">{error}</p>}
                        <p className="text-[10px] text-muted-foreground text-center">
                            Right-click an image & select <br /><b>"Copy Image Address"</b>
                        </p>
                    </div>
                </div>
            </div>
    ...
    return (
        ...
        >
            {data.url ? (
                ...
                <div className="h-full w-full flex items-center justify-center text-muted-foreground animate-pulse bg-muted/20">
                    {/* Placeholder while default loads */}
                </div>
            )}
